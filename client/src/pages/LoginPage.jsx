    ...
    <div className="flex w-full h-full lg:ml-24 px-10 py-10 justify-between place-items-center mt-20">
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 text-white w-full sm:w-full md:w-1/2 lg:w-1/3 px-7 py-7 rounded-xl justify-center align-middle shadow-xl">
        ...
        <form className="flex flex-col w-auto items-center" onSubmit={loginUser}>
            <h1 className='px-3 font-extrabold mb-5 text-white text-2xl'>Sign In</h1>
            ...
            <div className="input bg-gray-700 bg-opacity-50 border-gray-600">
              ...
              <input 
                type="email"  
                placeholder="Email" 
                className="input-et bg-transparent text-white placeholder-gray-400" 
                value={email} 
                onChange={ev => setEmail(ev.target.value)}
              />
            </div>

            <div className="input bg-gray-700 bg-opacity-50 border-gray-600 relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-gray-300">
              ...
              <input 
                ...
                className="input-et bg-transparent text-white placeholder-gray-400 pr-10" 
                value={password} 
                onChange={ev => setPassword(ev.target.value)}
              />
              <button 
                type="button" 
                className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 rounded-full hover:bg-gray-600 focus:outline-none"
            ...
            <div className='flex w-full h-full mt-4 justify-between px-1 text-gray-300'>
              <div className='flex gap-2'>
                <input type="checkbox" checked={rememberMe} onChange={() => setRememberMe(prev => !prev)}/> 
                Remember Me
              </div>
              <div>
                <Link to={'/forgotpassword'} className="text-gray-300 hover:text-white">
                  Forgot Password?
                </Link>
              </div>
            </div>
            ...
            <div className="text-center mt-4">
              ...
              <Link to={"/register"} className="text-primary hover:text-primarydark font-semibold">
                Sign Up
              </Link>
            </div>

            {/* Remove the entire container2 div since it's duplicate */}
            
            <Link to={'/'} className="">
              <button type="button" className="secondary"> {/* Changed to type="button" */}
